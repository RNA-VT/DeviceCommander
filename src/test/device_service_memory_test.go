...
func (s *InMemoryDeviceServiceSuite) SetupSuite() {
	utilities.ConfigureEnvironment()
	s.service = memory.InMemoryDeviceService{}
}

func (s *InMemoryDeviceServiceSuite) TestGet() {
...
func (s *InMemoryDeviceServiceSuite) TestDelete() {
	newDevs := GenerateRandomNewDevices(1)
	newDev := newDevs[0]
...
func (s *InMemoryDeviceServiceSuite) TestUpdate() {
	newDevs := GenerateRandomNewDevices(1)
	newDev := newDevs[0]
...
func TestInMemoryDeviceServiceSuite(t *testing.T) {
	suite.Run(t, new(InMemoryDeviceServiceSuite))
}
