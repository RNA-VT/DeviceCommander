...
func (_m *IDevice) RunHealthCheck(client device.IDeviceClient) (device.Device, error) {
	ret := _m.Called(client)

	var r0 device.Device
	if rf, ok := ret.Get(0).(func(device.IDeviceClient) device.Device); ok {
		r0 = rf(client)
	} else {
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(device.IDeviceClient) error); ok {
		r1 = rf(client)
	} else {
		r1 = ret.Error(1)
	}
